...
def register_professional():
    ...
    query_db("INSERT OR IGNORE INTO professionals (name,business_type,establishment,city,pin) VALUES (?,?,?,?,?)",
             (d["name"], d.get("business_type"), d.get("establishment"), d.get("city"), d.get("pin")))
...
def login_professional():
    d = request.json
    u = query_db(
        "SELECT * FROM users WHERE email=? AND password=? AND role='professional'",
...
def book():
    ...
    existing = query_db("SELECT * FROM appointments WHERE customer_phone=? AND professional=? AND date=? AND slot=?",
